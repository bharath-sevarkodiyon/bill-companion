...
import { FileUp, FileText, Sparkles, Loader2, CheckCircle2, Pencil, Copy } from "lucide-react";
import { toast } from "sonner";

const EXTRACT_URL = "https://abc.onrender.com/extract";

type Extracted = Record<string, any> & { submeters?: Array<Record<string, any>> };
...
const FileUploadTab = ({ onUploaded }: { onUploaded?: () => void }) => {
  ...
  const fieldEntries = data
    ? Object.entries(data).filter(
        ([k, v]) =>
          !["submeters", "justification", "overall_confidence_score"].includes(k) &&
          v !== null &&
          v !== undefined
      )
    : [];
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {/* Preview */}
      <Card className="bg-mist border-0 shadow-[var(--shadow-card)] overflow-hidden">
        <div className="bg-deep-navy text-primary-foreground px-5 py-3 flex items-center justify-between">
        ...
        <div className="p-5">
          <div className="aspect-[4/5] rounded-lg bg-[hsl(var(--ink))] flex items-center justify-center text-primary-foreground/30">
            {file ? (
              <FileText className="h-32 w-32" strokeWidth={1} />
            ) : (
      ...
      <Card
        ...
        onDrop={onDrop}
        className={`min-h-[420px] flex flex-col items-center justify-center text-center border-2 border-dashed transition-colors ${
          dragOver ? "border-deep-navy bg-mist/60" : "border-border bg-background"
        }`}
      >
        <input
          ref={inputRef}
        ...
        <p className="text-sm text-slate mt-2 max-w-xs">
          Drag and drop your PDF invoices, receipts, or technical documents here for instant extraction.
        </p>
        <Button
          variant="navy"
          className="mt-6"
        ...
        <p className="text-xs text-slate mt-3">Maximum file size: 25MB (PDF only)</p>
      </Card>
                ...
                <div className="pt-3">
                  ...
                  <div className="space-y-2">
                    {data.submeters.map((sm, i) => (
                      <div key={i} className="bg-background rounded-lg px-4 py-3">
                        ...
                        {Object.entries(sm)
                          .filter(([key]) => !["submeter_number", "consumption", "id"].includes(key))
                          .map(([key, val]) => (
                            <div key={key} className="flex justify-between text-xs text-slate mt-1">
                              <span>{key}</span>
